...
# Job readiness talks to the TV inside the daemon. When that conversation is
# refused, the runner reconnects and asks once more instead of failing the job.
my (@calls,@actions,$reconnects);
local *main::_log=sub {};
local *main::_log_action=sub {push @actions,$_[0]};
local *main::_save_job_readiness=sub {};
...
local *main::_api=sub {push @calls,$_[1];return shift @replies};
$reconnects=0;
my $result=main::_require_job_ready(0,{name=>'HDR10 Filmmaker'},'job');
...
ok(!eval {main::_require_job_ready(0,{name=>'Job'},'job');1},'a real readiness failure still stops the job');
like($@,qr/oledLight is not supported/,'with its cause');
