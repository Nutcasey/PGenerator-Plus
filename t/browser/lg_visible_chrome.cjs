...
(async()=>{
 ...
 page.on('request',req=>{
  const route=new URL(req.url()).pathname;let safe=['GET','HEAD'].includes(req.method())||['/api/lg/picture-settings','/api/automation/readiness'].includes(route);
 ...
 page.on('response',async res=>{
  const route=new URL(res.url()).pathname;if(!['/api/lg/picture-settings','/api/lg/picture-settings/set','/api/lg/verify-panel-light','/api/automation/readiness'].includes(route))return;
