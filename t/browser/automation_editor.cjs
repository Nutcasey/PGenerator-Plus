...
(async()=>{
 ...
 try{
  ...
  const results=await page.evaluate(()=>{
   ...
   check(initial.settings.contrast===85&&initial.settings.brightness===50&&initial.settings.peakBrightness==='off'&&initial.settings.colorGamut==='auto'&&initial.settings.dynamicColor==='off','new SDR jobs start with pinned reference picture controls');
  ...
  const defaultsChecks=await page.evaluate(async()=>{
   ...
   check(job.settings.contrast===100&&job.settings.peakBrightness==='high'&&job.settings.hdrDynamicToneMapping==='off'&&job.panel_light.fixed_value===100,'HDR starts with its own defaults, not SDR values');
   switchSignal('dv');job=pgAutomationRecipeFromForm();
   check(job.settings.contrast===100&&job.settings.peakBrightness==='high'&&!('colorGamut' in job.settings)&&!('hdrDynamicToneMapping' in job.settings),'DV defaults exclude decoder-managed gamut and tone mapping');
   ...
   try{
    ...
    check(job.settings.peakBrightness==='off'&&job.settings.energySaving==='off','missing TV values retain defaults');
  ...
  const gammaChecks=await page.evaluate(async()=>{
   ...
   pgAutomationEl('PictureMode').value='cinema';pgAutomationModeChanged();
   check(pgAutomationEl('Gamma').value==='2.2'&&pgAutomationRecipeFromForm().settings.gamma==='medium','SDR Cinema defaults its TV Gamma and LUT target together');
   pgAutomationEl('PictureMode').value='filmMaker';pgAutomationModeChanged();
