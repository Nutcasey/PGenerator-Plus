...
my @hdr_labels=(1.4,2,2.7,4,5,7,10,15,20,25,30,35,40,45,50,60,70,80,90,100);
my @hdr_rows=(14,19,28,42,51,70,103,154,206,257,308,360,411,462,514,612,715,817,920,1023);
for my $i (0..$#hdr_labels) {
 is(main::lg_autocal_hdr20_dpg_index_for_step({signal_mode=>'hdr10',pattern_signal_range=>1},
  {ire=>$hdr_labels[$i]}),$hdr_rows[$i],"HDR base $hdr_labels[$i]% row unchanged");
}
my %hdr_fillers=(1=>[73,11],2.3=>[84,23],3=>[90,30],3.7=>[96,37],6=>[117,62],8=>[134,82],
 55=>[546,563],65=>[633,664],75=>[721,767],85=>[809,870],95=>[896,972]);
for my $label (sort {$a<=>$b} keys %hdr_fillers) {
 my ($code,$row)=@{$hdr_fillers{$label}};
 is(main::lg_autocal_hdr20_dpg_index_for_step({signal_mode=>'hdr10',pattern_signal_range=>1},
  {ire=>$label,r=>$code,input_max=>1023}),$row,"HDR $label% filler follows legal code span");
}
is(main::lg_autocal_hdr20_dpg_index_for_step({signal_mode=>'hdr10',pattern_signal_range=>2},
 {ire=>3,r=>31,input_max=>1023}),31,'HDR full-range filler uses full source domain');
is(main::lg_autocal_hdr20_dpg_index_for_step({signal_mode=>'hdr10',pattern_signal_range=>1},
 {ire=>3,r=>23,input_max=>255}),33,'8-bit HDR filler uses 16..235 domain');
is(main::lg_autocal_hdr20_dpg_index_for_step({signal_mode=>'dv',pattern_signal_range=>2},
 {ire=>3,r=>361,input_max=>4095}),31,'DV legal 12-bit codes stay separate from full RGB transport');
is(main::lg_autocal_hdr20_dpg_index_for_step({signal_mode=>'hdr10'},
 {ire=>3,input_max=>1023}),undef,'missing filler code is not inferred from its label');
done_testing();
