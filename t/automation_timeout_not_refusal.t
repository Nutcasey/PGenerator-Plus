...
main::_reset_lg_control_seconds();
main::_note_lg_control_seconds(2,46,46);
is(main::_lg_control_seconds(),12,'a two-control timeout charges no session allowance to the controls');
...
# samples into the manifest for a resumed run.
{
 ...
 is(main::_lg_control_seconds(),12,'a write clamped to its 54 s budget teaches 12 s per control after the session allowance');
 is_deeply($saved[-1]{lg_control_samples},[12],'the sample is stamped into the manifest');
 ...
 is(main::_lg_control_seconds(),12,'a refused connection teaches nothing');
 is($calls[-1]{helper_timeout},66,'the next budget follows the measured figure');
 $reply={status=>'error',message=>'LG TV did not finish writing 3 picture controls within 66s.'};
 $step=400;
 main::_apply_settings_batched(0,$item,'c4',[sort keys %$settings],$settings,0);
 is(main::_lg_control_seconds(),15,'a helper that ran out of time teaches at most its budget, capped');
 my $stamped=$saved[-1];
 main::_reset_lg_control_seconds();
 is(main::_lg_helper_timeout_for($set,set_payload(3)),54,'a fresh process starts from the floor');
 is(main::_seed_lg_control_seconds($stamped),2,'the manifest samples reload at startup');
 is(main::_lg_helper_timeout_for($set,set_payload(3)),75,'and the resumed run keeps its measured budget');
